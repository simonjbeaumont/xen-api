...
let write_qemu_record domid uuid oss fd =
...
let suspend (task: Xenops_task.t) ~xc ~xs ~hvm xenguest_path domid fd flags ?(progress_callback = fun _ -> ()) ~qemu_domid do_suspend_callback =
	...
	let res =
		...
		(* Libxc record *)
		let oss = not (XenguestHelper.supports_feature xenguest_path "migration-v2") in
		debug "Writing Libxc%s header" (if oss then "_legacy" else "");
		write_header fd ((if oss then Libxc_legacy else Libxc), 0L) >>= fun () ->
		...
		* suspend-image-writing *)
		(if hvm then write_qemu_record domid uuid oss fd else return ()) >>= fun () ->
